...
async function getCognitiveRisk(patientId) {
  ...
  if (sessions.length < 6) return { success: true, sufficientData: false, overallStatus: "Insufficient data", riskLevel: "Insufficient data", confidence: 0, dataPoints: sessions.length, activityTrends: [], keyChanges: [], explanation: "At least 6 completed activity sessions are needed before a trend can be shown.", calculatedAt: new Date() };
  const activityTrends = Object.entries(activityNames).map(([gameType, name]) => {
    ...
    if (records.length < 4) return { gameType, name, status: "Insufficient data", dataPoints: records.length };
    ...
    return { gameType, name, status: change >= 5 ? "Improving" : change <= -5 ? "Declining" : "Stable", dataPoints: records.length, recentAccuracy: Math.round(recentAccuracy), previousAccuracy: Math.round(previousAccuracy), accuracyChange: change, responseTimeChange };
  });
  const measured = activityTrends.filter((trend) => trend.status !== "Insufficient data");
  if (!measured.length) return { success: true, sufficientData: false, overallStatus: "Insufficient data", riskLevel: "Insufficient data", confidence: 0, dataPoints: sessions.length, activityTrends, keyChanges: [], explanation: "More repeated sessions within an activity are needed to compare performance periods.", calculatedAt: new Date() };
